...
Description: Create the project using `uv` with a `pyproject.toml` configuration, install core dependencies (Django 5.x, pytest-django, python-dotenv), and initialize the Django project structure with a dedicated core app. Configure settings to support local environment variables and SQLite. Write and execute an initial automated smoke test to verify the test suite runs with `uv run pytest`.
Acceptance Criteria:
- [ ] Project initialized with `pyproject.toml` and managed via `uv`.
- [ ] Django 5.x, `pytest-django`, and `python-dotenv` installed and locked in `uv.lock`.
- [ ] Settings configured for local SQLite database and `.env` loading with `.env.example` provided.
- [ ] Initial smoke test runs and passes cleanly via `uv run pytest`.
...
Acceptance Criteria:
- [ ] `base.html` created with responsive navbar, message/toast container, and main content block.
- [ ] Tailwind CSS configured and compiled with clean utility styling.
- [ ] HTMX 2.x and Alpine.js 3.x loaded and functional.
- [ ] Home view created at `/` and verified with a passing test in `tests/test_home.py`.
...
Acceptance Criteria:
- [ ] `DataBankEntry` model defined with fields: `entry_type`, `title`, `organization`, `start_date`, `end_date`, `is_current`, `raw_text`, `metrics` (JSONField), and `tags` (JSONField).
- [ ] SQLite database migrations created and applied.
- [ ] Unit tests in `tests/test_models.py` pass and verify model CRUD operations, string methods, and tag indexing.
...
Acceptance Criteria:
- [ ] `DataBankEntry` registered in Django admin with search fields, list filters, and customized list display.
- [ ] Superuser login and permissions work locally.
- [ ] Automated tests verify that admin change list and edit views return HTTP 200 for staff users.
...
Acceptance Criteria:
- [ ] Data bank dashboard view lists all entries grouped by category with tag filter chips.
- [ ] HTMX-powered forms allow creating, editing, and deleting entries inline or via modal without full page reload.
- [ ] Automated tests in `tests/test_databank_views.py` verify creation, update, validation error handling, and deletion.
...
Acceptance Criteria:
- [ ] Ingestion service extracts clean text and sections from uploaded PDF and DOCX files.
- [ ] Parser transforms raw resume text into structured `DataBankEntry` candidate objects.
- [ ] Upload UI allows users to review and selectively save parsed items into their data bank.
- [ ] Automated tests verify document text extraction and parsing against sample test files.
...
Acceptance Criteria:
- [ ] Audit service evaluates data bank items and returns a structured quality report (missing metrics, vague phrasing, career gaps).
- [ ] UI view renders audit score, flagged items, and concrete recommendations.
- [ ] Automated tests verify audit evaluation logic and report generation with mock data.
...
Acceptance Criteria:
- [ ] `JobApplication`, `CVVersion`, and `ApplicationNote` models defined with appropriate relational constraints.
- [ ] Database migrations generated and applied.
- [ ] Unit tests verify relationships, version numbering increments, and cascade delete behavior.
...
Acceptance Criteria:
- [ ] Main dashboard displays all job application folders with company, role, date, and status tags.
- [ ] Modal/form allows creating a new application folder with pasted job post text and metadata.
- [ ] Application detail workspace page accessible via route `/applications/<id>/`.
- [ ] Automated view tests verify folder creation, validation errors, and listing.
...
Acceptance Criteria:
- [ ] AI client module initialized with environment variable configuration.
- [ ] Core Pydantic schemas defined for structured agent requests and responses.
- [ ] Helper functions wrap API calls with exception handling and JSON validation.
- [ ] Automated tests with mocked Anthropic responses verify schema parsing and error handling.
...
Acceptance Criteria:
- [ ] Agent evaluates job post against data bank entries and returns a structured sufficiency assessment.
- [ ] Returns `is_sufficient` boolean, missing core requirements list, and user guidance notes.
- [ ] Automated tests verify agent behavior with mock sufficient and insufficient datasets.
...
Acceptance Criteria:
- [ ] Agent drafts tailored CV sections with experience bullets strictly adhering to the XYZ structure.
- [ ] Prompt explicitly prohibits hallucinating unlisted achievements or metrics.
- [ ] Output conforms to structured Pydantic schema (`CVDraftSchema`).
- [ ] Automated tests verify schema parsing and structure validation.
...
Acceptance Criteria:
- [ ] Agent analyzes draft CV against target job post from the perspective of a senior recruiter.
- [ ] Returns structured output containing strictly three actionable rejection/critique reasons.
- [ ] Automated tests assert that output contains exactly three validated critique points.
...
Acceptance Criteria:
- [ ] Agent extracts core technical skills, certifications, and required keywords from job text.
- [ ] Calculates matched keywords, missing keywords, and match percentage score.
- [ ] Flags potential ATS formatting risks.
- [ ] Automated tests verify keyword extraction and matching calculations.
...
Acceptance Criteria:
- [ ] Agent revises the draft CV using feedback from Steps 2 and 3 without introducing unverified facts.
- [ ] Outputs revised CV structure and a list of unresolvable candidate gaps.
- [ ] Automatically creates a new `CVVersion` record linked to the `JobApplication`.
- [ ] Automated tests verify revision output schema and gap logging.
...
Acceptance Criteria:
- [ ] Agent calculates overall match score, fit tier (Strong Fit, Good Fit, Reach Role), and summary breakdown.
- [ ] Output saved to database as an `ApplicationNote` associated with the application.
- [ ] Automated tests verify score computation and persistence.
...
Acceptance Criteria:
- [ ] Pipeline orchestrator runs Steps 0 to 5 in sequential order.
- [ ] Streaming view pushes SSE messages (`step_start`, `step_complete`, `pipeline_finish`, `error`).
- [ ] Early exit triggered gracefully if Step 0 reports insufficient data.
- [ ] Automated tests verify sequential execution and event stream payload formatting.
...
Acceptance Criteria:
- [ ] Workspace UI includes a "Run Tailr Pipeline" action button connected to the SSE stream via HTMX.
- [ ] Visual step cards for Steps 0–5 reflect live states (Pending, Active, Complete, Failed).
- [ ] On pipeline completion, UI automatically reveals the generated CV and notes tabs.
- [ ] Automated view tests verify initial rendering and SSE connection markup.
...
Acceptance Criteria:
- [ ] In-browser editor allows direct editing of the CV sections and bullets.
- [ ] Save action creates a new `CVVersion` record marked as current.
- [ ] Version history dropdown allows viewing and restoring prior generated drafts.
- [ ] Automated tests verify version creation, retrieval, and switching.
...
Acceptance Criteria:
- [ ] Exporter generates a single-column `.docx` file formatted strictly for ATS compliance (no tables/text boxes).
- [ ] Download endpoint `/applications/<id>/export/docx/` delivers the `.docx` file with `application/vnd.openxmlformats-officedocument.wordprocessingml.document` content type.
- [ ] Automated tests verify file generation, non-empty binary output, and valid document structure.
...
Acceptance Criteria:
- [ ] HTML/CSS print template structured with ATS-safe styling (single column, standard fonts, clean margins).
- [ ] Exporter converts template to PDF with fully selectable text.
- [ ] Download endpoint `/applications/<id>/export/pdf/` delivers the `.pdf` file with `application/pdf` content type.
- [ ] Automated tests verify PDF creation and response status.
...
Acceptance Criteria:
- [ ] Notes tab in the workspace cleanly presents recruiter critiques, ATS keyword matches, and fit analysis.
- [ ] Separate export buttons allow downloading notes as `.md` or `.txt`.
- [ ] Downloaded notes contain full pipeline insights without being embedded into the CV file.
- [ ] Automated tests verify export endpoint response and notes formatting.
...
Acceptance Criteria:
- [ ] Status dropdown on application workspace allows updating status (`Drafted`, `Applied`, `Interviewing`, `Rejected`, `Offer`).
- [ ] Main dashboard allows instant filtering and searching of job folders by status and company name.
- [ ] Automated tests verify status updates and query filtering.
...
Acceptance Criteria:
- [ ] Workspace includes a toggle to optionally generate a tailored cover letter.
- [ ] Cover letter agent produces a narrative letter matching company tone and user qualifications.
- [ ] Generated cover letter displayed in a dedicated tab with independent PDF/DOCX export options.
- [ ] Automated tests verify cover letter agent generation and export endpoints.
